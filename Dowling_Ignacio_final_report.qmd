...
I chose to explore this data because I'm particularly interested in pro sports, but haven't conducted a comprehensive analysis to develop my own takeaways backed by data I explored. Thus, I chose to explore data regarding 30+ years of the NFL Draft, which is one of the hardest sports institutions for a fan or expert to identify a talented player... especially as the later rounds go on.

As mentioned in the executive summary, this EDA revolves around developing a more critical understanding of three concepts: career success, high draft status for linemen and quarterbacks, and high draft value for both teams and players.
...
An in-depth look at how I joined datasets and .csvs together -- as well as the difference in lengths for overlapping timelines for a few of these that affected how I explored the data -- can be found in the appendix.

:::


A total of four datasets are used here. Two are joined/merged, and two are raw. One of those raw ones contains four .csv files I used. Info regarding the topics of each are in the README file. All of them arise from two data sources from Kaggle, each of which can be found in the References section. Of the raw .csvs, only nfl_draft.csv comes from the dataset created by Pedersen^[See Reference 5.], while the other raw ones arise from Lichtenstein's ESPN Prospect Data.^[See Reference 4.]

nfl_draft.csv has 34 variables. 25 of those are numerical, with the other nine being categorical. It has 8,435 observations, and has missingness for many performance statistics as well as some attributes (such as age drafted). That is somewhat expected, since football players of different positions typically do not record statistics in every category.

nfl_draft_prospects.csv, which contains data for the players as draft prospects, has 13,354 observations and 24 variables (14 of which are categorical). The second contains textual information regarding prospects (much of which is contained in the first dataset) -- it has 12,905 observations and 21 variables (13 of which are categorical).

college statistics.csv has 66,672 observations with 13 variables; four are numerical and the other 9 are categorical. Finally, college_qbr.csv has 2,049 observations, and 14 variables (12 of those 14 are numerical). None appear to have missingness.

As for the joined datasets: nfl_draft_prospect_data_and_outcomes_1985_to_2015.csv has 46 variables, and 8,435 observations. Of those 46 variables, 30 are numerical, and 16 are categorical. It has missigness for statistical information at different positions (for the same reason as the nfl_draft.csv), as well as physical attributes and ESPN ranking data, which didn't exist for players drafted before 2004. There also was initial missigness in "team", which I used as the secondary identifier for a player's team name, but I resolved this by using the team abbreviation (which didn't have missingness) as the primary team identifier.

nfl_draft_prospects_and_profiles_combined_1985_to_2021 has 9,940 observations and 23 variables, 11 of which are numerical. There's significant missingness in the ESPN metrics and physical attributes for the same reasons mentioned above.
...
```
There seems to be, for the most part, a general decrease in median career AV by draft pick for the retired first-round picks. However, in the second round, there isn't an apparent decline in the median; the 64th overall pick has a range quite similar to that of the 35th pick. You could even say that the decline in median AV slows as early as, say, the 20th pick. 

So, it's evident that there's high value early for teams picking in the first round, and it tends to drop as the first round goes on. With first-round value looking high, that's a nice segue...
...
Since the roles of different positions in football vary drastically, let's look at this at a few positions. Given quarterbacks play the most influential position in the sport, and the steadily-declining potential value of an draft pick as the selection gets further away from No. 1, it's imperative to identify the most elite quarterback prospects. It begs the question:

#### Are the top QBs in college the highest-picked in the NFL Draft?

To get a sense of this, let's look at who the best quarterbacks were in college, statistically, since 2004. Let's start examining collegiate seasons with estimated points added (EPA), which is widely regarded^[See Reference 3.] as the best statistic to capture overall offensive impact, and then examine top seasons by quarterback ratings (QBR):
...
qb_data_to_2004 |>
  ...
  labs(title = "Distribution of College Estimated Points Added: QB Prospects 2004-2021", 
       x = "Estimated Points Added (EPA)")
...
nfl_draft_prospect_data_combined_1985_to_2021 |>
  ...
  ggplot(aes(x = overall, y = grade, color = player_name)) +
  geom_point(size = 4) +
  labs(title = "QBs with Top 25 College QBR Seasons Since 2004: Draft Pick vs. ESPN Grade", subtitle = "This does not include the nine players also ranked in Top 25 for College QBR since 2004.", x = "Overall Pick", y = "ESPN Draft Grade", color = "Player Name", caption = "All other players in the top 25 not listed here or in subtitle were not drafted.")
```
...
Let's examine height for quarterbacks. It's a metric that, at that particular position, has sparked lots of debate around prospects who aren't tall but possess other great skills^[See Reference 1]. Firstly, let's look at the height distribution for drafted QBs since 2004.
...
Now, looking at different players' long-term values:
...
So, let's bring a new metric into the fold: approximate value over expected (AVOE). I'll define it as a player's approximate value subtracted by the median AV for retired players near their draft spot. 

Granted, as we saw in some of the earlier explorations, the median AV for a player doesn't steadily decrease with every pick after the first round. Plus, different teams at different spots may pass on a slightly better player to fill a positional need, especially later in the draft. Thus, I'll designate the "median AV" compared to each player as the median AV of a five-spot cluster. For instance, a No. 1 overall pick's AV over expected in this EDA would be their approximate value minus the median AV of retired picks No. 1 through No. 5 since 1985. A No. 6 pick would be compared to the median AVs for overall picks 6-10, a No. 11 pick for a median among picks 11-15, etc.
...
```
Nearly every position in the sport is represented here (further, a nose tackle is often labeled as a defensive tackle)^[See Reference 2]. Linebackers and guards lead the way, though position doesn't necessarily appear to be a limiting factor of a player's ability to become a diamond in the rough.
...
## References
Bell, J. (2023, March 4). Bryce Young might ultimately prove to NFL his small build isn’t a problem | opinion. USA Today. https://www.usatoday.com/story/sports/nfl/columnist/bell/2023/03/03/bryce-young-weight-height-nfl-quarterback-draft-combine/11394682002/.

Football nose tackle. rookieroad.com. (n.d.). https://www.rookieroad.com/football/positions/nose-tackle/.

Jurgens, T. (2022, November 1). Betting 101: Explaining commonly cited metrics and factors in our handicapping: NFL and NCAA betting picks. PFF. https://www.pff.com/news/bet-nfl-bet-terms-metrics-game-script-handicapping-success-rate-epa#:~:text=Expected%20Points%20Added%20(EPA),-Expected%20Points%20Added&amp;text=For%20every%20play%2C%20EPA%20is,disparity%20in%20total%20plays%20run. 
